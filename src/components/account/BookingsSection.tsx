...
export default function BookingsSection({ bookings }: Props) {
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case "COMPLETED":
        return "bg-[#271024]/10 text-[#271024] dark:bg-[#e3ae72]/20 dark:text-[#e3ae72]";
      default:
  ...
  if (bookings.length === 0) {
    return (
      <Card className="border-[#271024]/10 dark:border-[#e3ae72]/20">
        <CardHeader>
          <CardTitle className="text-[#271024] dark:text-[#e3ae72]">My Bookings</CardTitle>
        ...
        <CardContent>
          <div className="flex flex-col items-center justify-center py-12 text-center">
            ...
            <p className="text-sm text-[#271024]/60 dark:text-[#e3ae72]/65 max-w-xs">
      ...
      <CardContent>
        <div className="space-y-4">
          {bookings.map((booking) => (
            <div
              key={booking.id}
              className="flex items-center gap-4 p-4 rounded-xl border border-[#271024]/10 dark:border-[#e3ae72]/15 bg-[#271024]/5 dark:bg-[#e3ae72]/5"
            >
              <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl bg-[#271024]/10 dark:bg-[#e3ae72]/20">
                <Calendar className="h-6 w-6 text-[#271024] dark:text-[#e3ae72]" />
              </div>
              <div className="flex-1 min-w-0">
                ...
                <p className="text-sm font-medium text-[#271024] dark:text-[#e3ae72]/90 truncate">
                  Service ID: {booking.serviceId}
                </p>
                <p className="text-xs text-[#271024]/60 dark:text-[#e3ae72]/60 flex items-center gap-1 mt-1">
                  <Clock className="h-3 w-3" />
                  {formatDateTime(booking.date)}
                </p>
                {booking.notes && (
                  <p className="text-xs text-[#271024]/60 dark:text-[#e3ae72]/60 mt-1 italic">
                    Note: {booking.notes}
                  </p>
                )}
              </div>
